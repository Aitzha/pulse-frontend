...
- **New-activity defaults** come from `nowKzTimeRoundedDown(5)` for start and `addMinutesToTime(start, 60)` for end. The only time constraint is that end must be strictly after start — enforced by `endBeforeStart` in the modal, which disables Add/Save and shows an inline danger message.
...
- **`TimePicker.vue`** is the shared hour/minute chooser. Chevron-up (top) decrements, chevron-down (bottom) increments; hour step 1, minute step 5; minute wrap does **not** carry into hour. `allow-end-of-day` extends the hour to 24.
